...
(defun cons-list (ls depth)
    (if (and (equal (length ls) 1) (< depth 2))
        (if (typep (car(cdr(car ls))) 'cons)
            (return-from cons-list (cons-list (cdr (car ls)) depth))
            (return-from cons-list (list (cons (car (car ls)) (car(cdr(car ls))))))
        )
    )
    ...
    (if (typep (car(cdr(car ls))) 'cons)
        (return-from cons-list (cons-list (cdr (car ls)) depth))
        (return-from cons-list (list (cons (car (car ls)) (car(cdr(car ls)))) (cons-list (cdr ls) depth)))
    )
...
;(print (evaluate '(let ((x 1)(y 1)) (+ x y)) nil))
